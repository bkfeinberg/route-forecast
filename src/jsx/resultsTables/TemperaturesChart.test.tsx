...
describe('TemperaturesChart component', () => {
  ...
  test('formats metric and imperial axis and tooltip values', () => {
    ...
    const cloudAxisProps = mockYAxisProps[0];
    const temperatureAxisProps = mockYAxisProps[1];
    const tooltipProps = mockTooltipProps[0];

    expect(xAxisProps.tickFormatter(10)).toBe('16');
    expect(cloudAxisProps.dataKey(mockChartData[0])).toBe('10');
    expect(temperatureAxisProps.tickFormatter(65)).toBe('105');
    expect(tooltipProps.labelFormatter(10)).toBe('6 km');
    expect(tooltipProps.formatter(65, 'feel')).toEqual(['105', 'feels like (C)']);
    expect(tooltipProps.formatter(80, 'Cloud cover')).toBe(80);

    render(<TemperaturesChart chartData={mockChartData} metric={false} popoverIsOpen={true} />);
    const imperialTooltipProps = mockTooltipProps[1];
    expect(imperialTooltipProps.labelFormatter(10)).toBe('10 miles');
    expect(imperialTooltipProps.formatter(65, 'feel')).toEqual([65, 'feels like (F)']);
  });
